...
function safeEval(expr: string, vars: Record<string, number>): number | null {
  ...
  for (const fn of ALLOWED_MATH) {
    safeMath[fn] = (Math as unknown as Record<string, unknown>)[fn];
  }
...
/** 检测值是否包含计算表达式 */
export function isComputedExpression(value: unknown): boolean {
...
function extractVariables(expr: string): string[] {
  ...
  const unique = new Set(
    matches.filter(
      (m) => m !== "Math" && !ALLOWED_MATH.has(m) && !JS_LITERALS.has(m),
    ),
  );
  return [...unique];
}
...
): { substituted: string; vars: Record<string, number>; missing: string[] } {
...
export function resolveComputedValue(
  ...
  if (!isComputedExpression(value)) return value;

  const strValue = String(value);

  // 提取所有 ${...} 表达式
  const result = strValue.replace(/\$\{([^}]+)\}/g, (match, expr: string) => {
...
export function resolveDisplayValue(
  ...
  if (isNaN(numValue)) return String(resolved);
