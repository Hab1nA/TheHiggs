...
const interactionPolicySchema = objectOf({
  ...
  commitOn: arrayOf(stringEnum(["blur", "enter", "change", "click", "submit"])),
  includeLocalStateOnCommit: booleanSchema,
  debounceMs: numberSchema,
}, ["mode"]);

const layoutHintsSchema = objectOf({
  width: stringEnum(["auto", "full", "content", "1/2", "1/3", "2/3", "1/4", "3/4"]),
...
const styleTokensSchema = objectOf({
  tone: stringEnum(["default", "muted", "primary", "success", "warning", "danger"]),
...
function node(
  ...
  return objectOf(
    { ...baseProperties(type), ...properties },
    ["id", "type", ...required],
  );
}
...
function terminalNodes(): JsonSchema[] {
  ...
  return [
    ...
    node("heading", { text: stringSchema, level: { enum: [1, 2, 3, 4] } }, ["text"]),
    ...
    node("code_block", { language: stringSchema, code: stringSchema }, ["code"]),
    node("table", {
      columns: arrayOf(stringSchema),
      rows: arrayOf(arrayOf({ anyOf: [stringSchema, numberSchema, booleanSchema, { type: "null" }] })),
    }, ["columns", "rows"]),
    node("button", {
      label: stringSchema,
      intent: stringSchema,
    ...
    node("select", {
      ...
      value: stringSchema,
      binding: stringSchema,
      options: arrayOf(objectOf({ label: stringSchema, value: stringSchema }, ["label", "value"])),
      interaction: interactionPolicySchema,
    }, ["binding", "options"]),
    node("checkbox", {
      label: stringSchema,
    ...
    node("chart_bar", {
      ...
      xLabel: stringSchema,
      yLabel: stringSchema,
      data: arrayOf(objectOf({ label: stringSchema, value: numberSchema }, ["label", "value"])),
    }, ["data"]),
    node("chart_line", {
      title: stringSchema,
      xLabel: stringSchema,
      yLabel: stringSchema,
      data: arrayOf(objectOf({ x: { anyOf: [stringSchema, numberSchema] }, y: numberSchema }, ["x", "y"])),
    }, ["data"]),
    node("badge", {
      text: stringSchema,
      variant: stringEnum(["default", "primary", "success", "warning", "danger", "info"]),
    ...
    node("progress", {
      ...
      max: numberSchema,
      unit: stringSchema,
      tone: stringEnum(["default", "primary", "success", "warning", "danger"]),
    }, ["value"]),
    node("statistic", {
      title: stringSchema,
      value: textValue,
    ...
    node("breadcrumb", {
      items: arrayOf(objectOf({ label: stringSchema, href: stringSchema }, ["label"])),
    ...
    node("description_list", {
      ...
      items: arrayOf(objectOf({ id: stringSchema, term: stringSchema, description: stringSchema }, ["id", "term", "description"])),
    }, ["items"]),
    node("empty_state", {
      icon: stringSchema,
      title: stringSchema,
      description: stringSchema,
      action: objectOf({ label: stringSchema, intent: stringSchema }, ["label", "intent"]),
    }, ["title"]),
    node("gauge", {
      title: stringSchema,
      value: numberSchema,
    ...
    node("color_swatch", {
      title: stringSchema,
      colors: arrayOf(objectOf({ value: stringSchema, label: stringSchema }, ["value"])),
      size: stringEnum(["sm", "md", "lg"]),
    }, ["colors"]),
    node("radar_chart", {
      title: stringSchema,
...
function recursiveNodes(childDepth: number): JsonSchema[] {
  ...
  return [
    node("screen", {
      title: stringSchema,
      layoutMode: stringEnum(["single", "dashboard", "workspace", "document", "wizard"]),
      gap: stringEnum(["none", "xs", "sm", "md", "lg"]),
      children,
    }, ["children"]),
    node("container", {
    ...
    node("region", {
      region: stringEnum(["header", "sidebar", "main", "inspector", "footer", "toolbar", "results", "logs"]),
      gap: stringEnum(["none", "xs", "sm", "md", "lg"]),
      children,
    }, ["region", "children"]),
    node("toolbar", {
...
const memoryPatchSchema = objectOf({
  session: arrayOf(objectOf({
    op: stringEnum(["add", "replace", "remove"]),
    path: stringSchema,
    value: unknownSchema,
  }, ["op", "path"])),
  app: arrayOf(objectOf({
    op: stringEnum(["add", "replace", "remove"]),
    path: stringSchema,
    value: unknownSchema,
  }, ["op", "path"])),
  userCandidates: arrayOf(objectOf({
    key: stringSchema,
    value: unknownSchema,
    reason: stringSchema,
