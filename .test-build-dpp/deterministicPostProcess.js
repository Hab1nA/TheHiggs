...
exports.deterministicPostProcess = deterministicPostProcess;
// -----------------------------------------------------------
// 交互节点类型集合
// -----------------------------------------------------------
...
function walkTree(node, visitor) {
    ...
    if (rec.primary && typeof rec.primary === "object") {
        walkTree(rec.primary, visitor);
    }
    if (rec.secondary && typeof rec.secondary === "object") {
        walkTree(rec.secondary, visitor);
    }
...
// Rule 1: 交互完整性修复
// -----------------------------------------------------------
function fixInteractionCompleteness(node, fixes) {
    walkTree(node, (n) => {
        const rec = n;
        const type = rec.type;
        // 所有交互节点必须有 semanticRole
        if (INTERACTIVE_TYPES.has(type) && !rec.semanticRole) {
            const defaultRole = type === "button" || type === "external_link" ? "navigation" : "input";
...
// Rule 2: 交互下限检查
// -----------------------------------------------------------
function fixInteractiveMinimum(node, fixes) {
    ...
    if (interactiveCount === 0) {
        ...
        walkTree(node, (n) => {
            const r = n;
            if (r.type === "screen" && !screenNode)
                screenNode = r;
        });
        // 如果没有 screen，就用根节点
        const target = screenNode ?? node;
        if (Array.isArray(target.children)) {
            ...
            fixes.push({
                ...
                description: "Screen had 0 interactive controls, added a navigation button",
                after: "返回首页",
            });
        }
    }
...
// Rule 3: 间距一致性修复
// -----------------------------------------------------------
function fixSpacingConsistency(node, fixes) {
    walkTree(node, (n) => {
        const rec = n;
        const type = rec.type;
        // 容器节点应有 gap
...
// Rule 4: 可访问性修复
// -----------------------------------------------------------
function fixAccessibility(node, fixes) {
    walkTree(node, (n) => {
        const rec = n;
        const type = rec.type;
        // image 节点必须有 alt
...
// Rule 5: 样式一致性修复
// -----------------------------------------------------------
function fixStyleConsistency(node, fixes) {
    // 统计 screen 下所有直接子节点的 density
    const screen = node;
    if (screen.type !== "screen" || !Array.isArray(screen.children))
        return;
    const densityCount = {};
...
// -----------------------------------------------------------
function fixLocalActionTargets(node, fixes) {
    ...
    walkTree(node, (n) => {
        ...
        if (la.type === "append_text" && typeof la.targetBinding === "string") {
            ...
            walkTree(node, (target) => {
                const tr = target;
                if ((tr.type === "text_input" || tr.type === "textarea") &&
...
function fixTabButtonWiring(node, fixes) {
    ...
    walkTree(node, (n) => {
        ...
        for (const [, tabsInfo] of tabsMap) {
            const matchedTab = tabsInfo.tabs.find((tab) => tab.label.trim().toLowerCase() === buttonLabel);
            if (!matchedTab)
                continue;
 ...
 * @returns 修复结果
 */
function deterministicPostProcess(ui) {
    const fixes = [];
    // 按优先级顺序执行修复规则
